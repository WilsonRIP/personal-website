...
const buttons: Button[] = [
  { value: "C", type: "clear", className: "col-span-2", keyboardKey: ["Escape", "Delete"] }, // AC/Clear
  { value: "DEL", type: "delete", className: "col-span-2", keyboardKey: "Backspace" }, // Delete
...
const Calculator: React.FC = () => {
  ...
  const handleOperator = (op: string) => {
      setState((prevState) => {
          ...
          if (currentOperand == null || (currentOperand === "-" && previousOperand == null)) {
          ...
          // Handle changing operator if pressed consecutively
           if (previousOperand != null && currentOperand === "0" || currentOperand === null) { // Changed logic: if previous operand exists but current is effectively empty
                if (op === '-' && operation !== '-') { // Allow changing to minus for negative numbers
                     // Don't change state operation yet, just modify current input
                     return { ...prevState, currentOperand: '-' };
                 } else if (op !== '-') { // Replace the operation
                     showFeedback("Operator changed");
                     return { ...prevState, operation: op, currentOperand: null, overwrite: true }; // Expect next number
                 }
          ...
          if (previousOperand != null && operation != null && !prevState.overwrite) { // Added !prevState.overwrite check
  ...
  const handleDelete = () => {
      setState((prevState) => {
          ...
          if (prevState.currentOperand === "0" || prevState.currentOperand == null || prevState.currentOperand.length === 1) {
  ...
  const handleButtonClick = (value: string, type: ButtonType) => {
    ...
    if (type !== "equals" && feedbackMessage !== "Cannot divide by zero" && feedbackMessage !== "Error" && feedbackMessage !== "Invalid input") {
         setFeedbackMessage("");
         if (feedbackTimeoutRef.current) {
              clearTimeout(feedbackTimeoutRef.current);
              feedbackTimeoutRef.current = null;
     ...
     if (state.currentOperand === "Error" && (type === 'number' || type === 'clear' || type === 'decimal')) {
         handleClear();
    ...
    <div className="w-full max-w-full sm:max-w-md md:max-w-lg mx-auto rounded-xl bg-white p-4 sm:p-6 shadow-2xl dark:bg-gray-800 transition-all duration-300">
      ...
      >
         ...
         <div className="text-xs sm:text-sm opacity-60 truncate">
            {state.previousOperand && state.operation ? `${formatOperand(state.previousOperand)} ${state.operation}` : ''}
         </div>
         {/* Main Display */}
